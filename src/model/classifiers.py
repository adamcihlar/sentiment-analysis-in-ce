...
class AdaptiveSentimentClassifier:
        ...
        if k == 1:
            ...
            if radius:
                ...
                knn = RadiusNeighborsClassifier(
                    radius=rad, weights="distance", outlier_label=999
                )
            else:
    ...
    def suggest_anchor_set(
        ...
        Cluster using kmeans to set the number of samples to label.
        ...
        if anchor_set_size is None:
            logger.warning("Number of samples to label not provided - set to 100.")
            anchor_set_size = 100
