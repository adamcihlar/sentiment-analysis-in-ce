...
def main(datasets):
    ...
    for dataset in datasets:
        path_to_results = os.path.join(paths.OUTPUT_INFO_ZEROSHOT)
        re_pattern = "^" + dataset
...
if __name__ == "__main__":
    ...
    mall = pd.read_csv("output/train_info/zeroshot/mall.csv", index_col=0)
    facebook = pd.read_csv(
        "output/train_info/zeroshot/facebook.csv", index_col=0
    )
    csfd = pd.read_csv("output/train_info/zeroshot/csfd.csv", index_col=0)

    mall_or = pd.read_csv("output/train_info/finetuning/test/mall.csv", index_col=0)
    facebook_or = pd.read_csv(
        "output/train_info/finetuning/test/facebook.csv", index_col=0
    )
    csfd_or = pd.read_csv("output/train_info/finetuning/test/csfd.csv", index_col=0)
